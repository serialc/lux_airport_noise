...
## Problem ~ 1500 pages like this
![](imgs/pdf_example.png "A screenshot of a PDF page with headings, a table, and graphs"){width=100%}
...
## The data
```{r, echo=FALSE}
pp <- read.table(file = '../export_csv/2020_airport_noise_data2.csv', header=TRUE)
...
## Relationship between events and noise
```{r}
...
legend("bottomrigh", legend = unique(pp$location), fill=factor(unique(pp$location)))

```

I'm not sure where the locations are exactly.
...
::: {.column width="50%"}
### Next steps
- Process the other years of data (2012-2021)
- Analyze the data
:::

::: {.column width="50%"}
### PDFS
- Relatively easy to parse (here)
- A nice visual and methodologically interesting task
:::
::::
